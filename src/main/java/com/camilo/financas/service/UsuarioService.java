...
public class UsuarioService {
    ...
    public void deletar(UUID id) {
        ...
        Usuario usuario = usuarioRepository.findById(id).orElseThrow(() -> new CampoInvalidoException("User not found with id " + id));

        if (!usuario.getGastos().isEmpty()) {
            throw new CampoInvalidoException("email ja em uso!");
        }
